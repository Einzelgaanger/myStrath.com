...
console.log(`${colors.blue}Starting simplified build process...${colors.reset}`);
...
try {
  ...
  // Step 3: Copy server files directly without TypeScript compilation
  ...
  try {
      ...
      for (const file of files) {
        ...
        if (stats.isDirectory()) {
          ...
          const jsContent = fs.readFileSync(sourcePath, 'utf-8')
            .replace(/import .* from ["'](.*)\.ts["'];?/g, 'import * from "$1.js";')
            .replace(/import \{(.*)\} from ["'](.*)\.ts["'];?/g, 'import {$1} from "$2.js";')
            .replace(/\.ts/g, '.js');
          
          fs.writeFileSync(targetPath.replace('.ts', '.js'), jsContent);
        } else {
    ...
    console.log(`${colors.green}Server files copied successfully!${colors.reset}`);
  } catch (error) {
    console.error(`${colors.red}Error copying server files:${colors.reset}`, error.message);
    process.exit(1);
  }

  // Step 4: Copy shared files
  console.log(`${colors.yellow}Copying shared files...${colors.reset}`);
  ...
  try {
    ...
    console.log(`${colors.green}Shared files copied successfully!${colors.reset}`);
  } catch (error) {
    console.error(`${colors.red}Error copying shared files:${colors.reset}`, error.message);
    process.exit(1);
  }
